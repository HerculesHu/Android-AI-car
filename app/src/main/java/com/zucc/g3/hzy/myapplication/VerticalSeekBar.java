...
public class VerticalSeekBar extends FrameLayout
{
    ...
    private int mMax = 1024;
    private int   mMin=0;
    private boolean mTrackingTouch = false;
    private OnSeekBarChangeListener mOnSeekBarChangeListener;
    private int mBGSize =100;          //进度条大小
    private int mBGRadius=20;     //进度条圆角
    private int mCircleSize=150;       //圆的大小
    ...
    private void setProgress(int progress, boolean fromUser)
    {
        ...
        else if(10<=progress&&progress<256){
            mTextView.setText("龟速");
        }
        else if(256<=progress&&progress<512){
            mTextView.setText("低速");
        }
        else if(512<=progress&&progress<768){
            mTextView.setText("中速");
        }
        else if(768<=progress&&progress<=1024){
            mTextView.setText("高速");
        }
